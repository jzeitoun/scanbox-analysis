...
def construct(fh, data, spec):
    """ Data structure assumed to be in TZCYX format. """

    per_timepoint_spec = spec['per_timepoint']
    per_channel_spec = spec['per_channel']
    per_file_spec = spec['per_file']
    root_attributes = spec['root_attrs']

    num_timepoints, _, num_channels, _, _ = data.shape
    num_tasks = num_timepoints * num_channels

    per_timepoint_status = Statusbar(num_tasks, title='Processing frames...',
            mem_monitor=True)
    per_timepoint_status.initialize()

    # First iterate through frames.
    for t, timepoint in enumerate(data):
        for c, frame in enumerate(timepoint.transpose(1, 0, 2, 3)):
            ...
            apply_spec(fh, frame, per_timepoint_spec, axes_lookup)
            per_timepoint_status.update(t*num_channels+c+1)
