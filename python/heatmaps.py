...
def generate_heatmaps(df, eye):

    rows,cols = 50,50

    series = df['velocity'][df['eye'] == eye]
    velocity_xy = np.concatenate(series.as_matrix())
    velocity_xy = velocity_xy[~np.isnan(velocity_xy)[:,0]] # remove NaN values
    high_count = len(velocity_xy[velocity_xy > 50])
    print 'High Count:', high_count
    velocity_xy[velocity_xy > 50] = 0
